...
import {
  listOutgoing, listIncoming, shareCache,
  discoverFromContacts, contactsAvailable, SHARE_CATEGORIES,
  type OutgoingShare, type IncomingShare, type AppUser, type InviteContact,
...
const openShareConfig = (id: string, name: string, share?: OutgoingShare) =>
  router.push({
    ...
    params: {
      id, name,
      sel: JSON.stringify({ categories: share?.categories ?? [], items: share?.items ?? {} }),
    },
  });

const catLabel = (k: string) => SHARE_CATEGORIES.find((c) => c.key === k)?.label ?? k;
...
export default function SharingScreen() {
  ...
  // ── Data ─────────────────────────────────────────────────────────────────────
  const [outgoing, setOutgoing]         = useState<OutgoingShare[]>(shareCache.data?.out ?? []);
  const [incoming, setIncoming]         = useState<IncomingShare[]>(shareCache.data?.inc ?? []);
  ...
  const loadShares = useCallback(async () => {
    if (!apiEnabled) { setLoadingShares(false); return; }
    ...
    } catch { /* keep stale */ } finally { setLoadingShares(false); }
  }, []);

  useFocusEffect(useCallback(() => { void loadShares(); }, [loadShares]));

  const loadContacts = useCallback(async () => {
    if (!contactsAvailable) { setContactsState("unavailable"); return; }
    setContactsState("loading");
    try {
      const res = await discoverFromContacts();
      setNetUsers(res.users); setInvitable(res.invitable);
  ...
  const inviteContact = (c: InviteContact) => {
    Share.share({
      message: `Hey ${c.name.split(" ")[0]}! I use NetWorth to track my finances. Join me — https://getworthapp.com`,
  ...
  return (
    <SafeAreaView edges={["top", "bottom"]} className="flex-1 bg-cosmic-darker">
      ...
      <View
        ...
        onLayout={(e) => { tabBarW.value = e.nativeEvent.layout.width; }}
      >
        ...
        <Animated.View
          style={[
            { position: "absolute", bottom: 0, height: 2, backgroundColor: "#a855f7", borderRadius: 2 },
            indicatorStyle,
          ]}
        />
      </View>
                  ...
                  {outgoing.length === 0 ? (
                    ...
                    outgoing.map((s, i) => (
                      <React.Fragment key={s.recipient.id}>
                        <TouchableOpacity onPress={() => openShareConfig(s.recipient.id, s.recipient.name, s)} activeOpacity={0.7} className="flex-row items-center gap-3 px-xl py-3">
                          <Avatar name={s.recipient.name} />
                          <View className="flex-1">
                            <Text className="text-base font-semibold text-white">{s.recipient.name}</Text>
                            <Text className="text-xs text-dim" numberOfLines={1}>{s.categories.map(catLabel).join(", ")}</Text>
                          </View>
                          <Ionicons name="chevron-forward" size={16} color="#374151" />
                        </TouchableOpacity>
                        {i < outgoing.length - 1 && <View className="h-px bg-white/[0.04] ml-[76px] mr-xl" />}
                      </React.Fragment>
                    ))
                  )}
                  ...
                  {incoming.length === 0 ? (
                    ...
                    incoming.map((s, i) => (
                      <React.Fragment key={s.owner.id}>
                        <TouchableOpacity onPress={() => router.push(`/shared/${s.owner.id}`)} activeOpacity={0.7} className="flex-row items-center gap-3 px-xl py-3">
                          <Avatar name={s.owner.name} />
                          <View className="flex-1">
                            <Text className="text-base font-semibold text-white">{s.owner.name}</Text>
                            <Text className="text-xs text-dim" numberOfLines={1}>{s.categories.map(catLabel).join(", ")}</Text>
                          </View>
                          <Ionicons name="chevron-forward" size={16} color="#374151" />
                        </TouchableOpacity>
                        {i < incoming.length - 1 && <View className="h-px bg-white/[0.04] ml-[76px] mr-xl" />}
                      </React.Fragment>
                    ))
                  )}
                </ScrollView>
            ...
            <View style={{ width }}>
              {contactsState === "idle" || contactsState === "loading" ? (
                <View className="flex-1 items-center justify-center gap-3">
                  <ActivityIndicator color="#a855f7" />
                  <Text className="text-sm text-muted">Finding contacts on NetWorth…</Text>
                </View>
              ) : contactsState === "denied" ? (
                <View className="flex-1 items-center justify-center px-8 gap-3">
                  <View className="w-14 h-14 rounded-2xl items-center justify-center border border-white/[0.08]" style={{ backgroundColor: "rgba(255,255,255,0.05)" }}>
                    <Ionicons name="lock-closed-outline" size={26} color="#6b7280" />
                  </View>
                  <Text className="text-base font-semibold text-white text-center">Contacts permission needed</Text>
                  <Text className="text-sm text-muted text-center">
                    Allow contacts access to find friends on NetWorth. Their numbers are hashed on-device.
                  </Text>
                  <TouchableOpacity
                    onPress={() => { contactsLoaded.current = false; void loadContacts(); }}
                    className="px-6 py-3 rounded-2xl mt-1"
                    style={{ backgroundColor: "rgba(168,85,247,0.9)" }}
                  >
                    <Text className="text-sm font-bold text-white">Grant Permission</Text>
                  </TouchableOpacity>
                </View>
              ) : contactsState === "unavailable" ? (
                <View className="flex-1 items-center justify-center px-8">
                  <Text className="text-sm text-muted text-center">Contact discovery requires a native build of the app.</Text>
                </View>
              ) : (
                <ScrollView nestedScrollEnabled showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
                  <Text className="text-xs font-bold text-muted uppercase tracking-widest px-xl mt-5 mb-1">
                  ...
                  {netUsers.length === 0 ? (
                    <View className="px-xl py-4">
                      <Text className="text-sm text-dim">None of your contacts are on NetWorth yet.</Text>
                    </View>
                  ) : (
                    netUsers.map((u, i) => {
                        ...
                        <React.Fragment key={u.id}>
                          ...
                          >
                            ...
                            <View className="flex-1">
                              ...
                              <Text className="text-xs text-dim">{already ? "Tap to edit sharing" : "On NetWorth"}</Text>
                            </View>
                            {already && (
                              <View className="px-3 py-1 rounded-full" style={{ backgroundColor: "rgba(34,197,94,0.15)" }}>
                                <Text className="text-xs font-semibold" style={{ color: "#22c55e" }}>Sharing</Text>
                              </View>
                            )}
                          </TouchableOpacity>
                          {i < netUsers.length - 1 && <View className="h-px bg-white/[0.04] ml-[76px] mr-xl" />}
                        </React.Fragment>
                      );
                    })
                  )}
                    ...
                    <>
                      ...
                      {invitable.map((c, i) => (
                        <React.Fragment key={`${c.name}-${i}`}>
                          <TouchableOpacity onPress={() => inviteContact(c)} activeOpacity={0.7} className="flex-row items-center gap-3 px-xl py-3">
                            <Avatar name={c.name || "?"} />
                            <View className="flex-1">
                              <Text className="text-base font-semibold text-white">{c.name || c.phone}</Text>
                              <Text className="text-xs text-dim">{c.phone}</Text>
                            </View>
                            <View className="px-3 py-1 rounded-full border border-white/[0.1]" style={{ backgroundColor: "rgba(255,255,255,0.05)" }}>
                              <Text className="text-xs font-semibold text-muted">Invite</Text>
                            </View>
                          </TouchableOpacity>
                          {i < invitable.length - 1 && <View className="h-px bg-white/[0.04] ml-[76px] mr-xl" />}
                        </React.Fragment>
                      ))}
                    </>
                  )}
